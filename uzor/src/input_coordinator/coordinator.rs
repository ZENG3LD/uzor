...
use super::response::WidgetResponse;
use crate::input::state::InputState;
use super::widget_state::WidgetInputState;
use super::text_field::TextFieldStore;
...
pub struct InputCoordinator {
    ...
    /// Text field store — manages all text input state
...
impl InputCoordinator {
    ...
    /// Access the text field store (read-only)
    pub fn text_fields(&self) -> &TextFieldStore {
        &self.text_fields
    }

    /// Access the text field store (mutable)
...
mod tests {
    ...
    #[test]
    fn test_text_fields_wired() {
        use super::super::text_field::TextFieldConfig;
        let mut coord = make_coordinator();
        let id = WidgetId::new("search_box");
        coord.text_fields_mut().register(id.clone(), TextFieldConfig::text());
        coord.text_fields_mut().focus(&id);
